...
#define IS_PROXY(x) (x == proxy_upstream_ascii_prot || \
                     ...
                     x == proxy_downstream_binary_prot || \
                     x == negotiating_proxy_prot)
#define IS_DOWNSTREAM(x) (x == proxy_downstream_ascii_prot || \
                          x == proxy_downstream_binary_prot)
...
typedef struct {
    ...
    SOCKET notify_receive_fd;      /* receiving end of notify pipe */
    SOCKET notify_send_fd;         /* sending end of notify pipe */
...
conn *conn_new(const SOCKET sfd, const enum conn_states init_state,
...
void dispatch_conn_new(SOCKET sfd, enum conn_states init_state,
                       int event_flags,
